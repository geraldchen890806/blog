...
function format(value, formatString, options) {
  ...
  // 处理小数精确度
  const precision = formatString.length - formatString.indexOf('.') - 1;
  ...
  if (doubule) {
    return format(output, formatString, compare);
  }
  return output;
}
...
function formatter(formatString, compare) {
  return (value) => format(value, formatString, compare);
}
