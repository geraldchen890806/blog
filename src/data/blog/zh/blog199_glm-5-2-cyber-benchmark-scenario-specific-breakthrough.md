...
tags:
  ...
  - 安全
  - 开发效率
description: 智谱 GLM 5.2 在 Semgrep IDOR 漏洞检测基准 F1 拿 39%，超过 Claude Code Opus 4.8（28%）11 个点、Opus 4.6（37%）2 个点，单次发现成本约 1/6。但综合 benchmark 它仍比 Claude Opus 系差约 9 分。这种"综合差一截、特定场景反超"格局在 Kimi K2.5、Qwen3-Max 同期复现。本文拆"场景特化超越"是不是 2026 中国模型新拐点。
---
...
## 数据全貌：GLM 5.2 综合差 9 分，特定场景却赢

放在更大坐标系里看 GLM 5.2 真实位置：

| 维度 | GLM 5.2 | Claude Opus 4.8 / Fable 5 / 4.5 |
|---|---|---|
...
读这张表的正确顺序是：
1. **综合 benchmark 仍落后**——91 vs ~93，约 9 分差距。任何把 GLM 5.2 说成"全面超越 Claude"的文章都是夸大。
...
| **GLM 5.2** | Design Arena 人类盲评 | #1，领先 Claude Fable 5 10 Elo（Fable 5 当时已下线，需注意时序） |
...
- 不跟 Claude/GPT 比"综合都强"——综合榜 9 分差距短期内追不上
...
后者**只有在开源模型本身基线足够强**时才成立。2026 年这件事第一次满足——综合基线落后 9 分，但已到"特化后能反超"的门槛。
...
普通对话场景，每次 query token 量小，Claude Opus 5x 贵不致命。但 **agent 时代单次任务可能 100k-1M token**，5x 价差 + 长时长跑就是**月账单从 $500 → $4000**。这时候"GLM 5.2 综合差 9 分但在我这场景跟 Claude 持平"，就是**理性采购决策**。
...
**反对"全面拐点"的证据**：
- 综合 benchmark 仍落后 9 分，**这意味着"通用任务"市场 Claude 还稳**
