...
title: 工具指南 #1：在线图片压缩 — 无损压缩、隐私安全、无需上传
...
tags:
  ...
  - 前端
  - 性能优化
description: 详解纯前端在线图片压缩工具的使用方法，支持 PNG/JPEG/WebP 格式，本地压缩不上传服务器，附带图片压缩的技术原理和实际应用场景。
---
...
市面上 TinyPNG、Squoosh 等工具都很好用，但纯前端方案有几个独特优势：
...
多数在线工具的免费版有每日次数限制或单张大小限制。纯前端方案没有服务器成本，自然也没有这些限制。你可以一次压缩 50 张图片，每张 20 MB，完全没问题。

### 离线也能用

页面加载完成后，断网也能正常使用。出差途中没有 Wi-Fi？只要之前打开过这个页面，照样可以压缩图片。
...
- **减少颜色数**：如果一张 PNG 只用了 100 种颜色，可以把它从 24 位真彩转为 8 位索引色，大幅减小文件
...
同等画质下，WebP 比 JPEG 小 25-34%，比 PNG 小 26%。目前主流浏览器都已经支持 WebP，是网页图片的推荐格式。
...
以一张 4032x3024 的 iPhone 照片（原始大小 4.2 MB JPEG）为例：
